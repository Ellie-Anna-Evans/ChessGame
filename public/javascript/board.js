...
class Board{
    ...
    setupBoard(){
        ...
        var i;
        for(i = 1; i <= 8; i++){
            this.board[1].push(new Space(new Pawn("black"), 'B', i));
        }

        for(i = 1; i <= 8; i++){
            this.board[2].push(new Space("none", 'C', i));
        }

        for(i = 1; i <= 8; i++){
            this.board[3].push(new Space("none", 'D', i));
        }

        for(i = 1; i <= 8; i++){
            this.board[4].push(new Space("none", 'E', i));
        }

        for(i = 1; i <= 8; i++){
            this.board[5].push(new Space("none", 'F', i));
        }

        for(i = 1; i <= 8; i++){
            this.board[6].push(new Space(new Pawn("white"), 'G', i));
        }
